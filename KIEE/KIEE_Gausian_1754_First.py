...
# 선간 단락 전류 정규 분포 
norm_dist = scipy.stats.norm(loc = averageA, scale = stdA)
...
plt.scatter(0, norm_dist.pdf(0), c = 'r') # x = 0인점 표시
plt.plot([-0.1, 0], [norm_dist.pdf(0), norm_dist.pdf(0)], 'r--')
plt.plot([-0.1, 0], [norm_dist.pdf(0), norm_dist.pdf(0)], 'r--')
...
# 1선 지락 전류 정규 분포 
norm_dist = scipy.stats.norm(loc = averageB, scale = stdB)
